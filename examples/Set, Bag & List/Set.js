 ...
 * A.top(count:Number, f(x)|attribute):
 *   Creates a sorted list with at most count elements sorted by decreasing order of the result of f(x) or attribute.
 * A.bottom(count:Number, f(x)):
 *   Creates a sorted list with at most count elements sorted by increasing order of the result of f(x) or attribute.
 * A.sort( f(x)):
 ...
 * A.appendAll(B): Appends all elements of B to the A set. Returns A set.
...
//---------------
    ...
    Assertions.assertEquals(oneToFive, arrayToSet([1, 2, 3, 4, 5]))

    var doubleOneToFive = Set(2, 4, 6, 8, 10)

    var mix = Set();
    mix.appendAll(oneToFive);
    mix.appendAll(doubleOneToFive);
    Assertions.assertEquals(mix, oneToFive ∪ doubleOneToFive)
